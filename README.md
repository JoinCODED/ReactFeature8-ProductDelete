...
1. In `styles.js`, create a styled component called `DeleteButtonStyled`. Instead of a button I'll use a `p` tag, our tag's color will be red.
   ...
   Don't forget to export this component!

2. But we don't have a `red` color in our theme! In `App.js`, we will add our shade of red to both themes.
...
3. Import `DeleteButtonStyled` in `CookieItem` and render it under the cookie's price
...
4. Check the browser, our `delete` is showing! The question is, can we add an `onClick` event on a `p` tag? Yes you can. You can add an `onClick` event on any tag.

5. Let's create a method that will handle the deleting in `CookieItem`, and for now let's give it an alert with the cookie's ID:

   ```javascript
   const handleDelete = () => {
     alert(`Delete cookie #${props.cookie.id}`);
   };
   ```
...
3. In `CookieItem`, we will pass `props.deleteCookie` to `handleDelete`:
   ...
   ```javascript
   const handleDelete = () => {
     props.deleteCookie(props.cookie.id);
   };
   ```
...
9. Let's fix our `deleteCookie` method to use our state and state method:
   ...
   ```javascript
   const deleteCookie = (cookieId) => {
     const updatedCookies = _cookies.filter(
